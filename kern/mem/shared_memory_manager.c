...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	struct Share *shareddd;
	size = ROUNDUP(size,PAGE_SIZE);
	int num_p = (size/PAGE_SIZE);
	int ind= ((uint32)virtual_address/PAGE_SIZE);
     if (get_share_object_ID(ownerID, shareName) != E_SHARED_MEM_NOT_EXISTS)
	 		return E_SHARED_MEM_EXISTS;
 	 int sh_id = allocate_share_object(&shareddd);
     if(sh_id == E_NO_SHARE)
	 		return E_NO_SHARE;
     if(get_share_object_ID(ownerID, shareName) == E_SHARED_MEM_NOT_EXISTS)
     {
    	 ...
    	 uint32 st;
    	 for( st=va; st< end; st+=PAGE_SIZE ) {
    		 ptr_frame= get_frame_info(curenv->env_page_directory, va,&ptr_page_table_va);
    		 if (ptr_frame!= NULL) {
    			 return -1;
    		 }
    		 if (ptr_frame==NULL)
    		 {
    			 int ret =get_page_table( curenv->env_page_directory, st, &ptr_page_table_va);
    			 if ( ret == TABLE_NOT_EXIST ) {
    				 ptr_page_table_va= create_page_table(curenv->env_page_directory,st) ;
    			 }
    		 }
    	 ...
    	 for( st=va; st< end; st+=PAGE_SIZE ) {
    		 int rett= allocate_frame(&ptr_alloc);
                 uint32 index = (st/PAGE_SIZE);
    			 map_frame(curenv->env_page_directory,ptr_alloc,st,PERM_WRITEABLE|PERM_USER);
    			 add_frame_to_storage(shares[sh_id].framesStorage , ptr_alloc , i);
    			 i++;
    	 }
    	        shares[sh_id].empty = 0;
    	 		strcpy(shares[sh_id].name, shareName);
    	 		shares[sh_id].size = size;
    	 		shares[sh_id].isWritable = isWritable;
    	 		shares[sh_id].references = 1;
    	 		shares[sh_id].ownerID = ownerID;

     }
     //cprintf("id : %d", sh_id);
     //cprintf("\nsize : %d", shares[sh_id].size);
	return sh_id;
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
			...
			struct Share *sarr =&shares[index];
			//cprintf("\nid =%d", index);
			...
			if(index != E_SHARED_MEM_NOT_EXISTS)
			{
				uint32 fmax=shares[index].size;
				//cprintf("\nsize =%d", fmax);
				uint32 end=ROUNDDOWN(((uint32)virtual_address+fmax),PAGE_SIZE);
				int a=0;
				for(uint32 i=start ; i<end ; i+=PAGE_SIZE)
				{

						//F=NULL;
							F=get_frame_from_storage(sarr->framesStorage,a);
							if(shares[index].isWritable){
								map_frame(myenv->env_page_directory,F,i,PERM_USER | PERM_WRITEABLE | PERM_PRESENT);
							}
							else{
								//int perm = 0;
								map_frame(myenv->env_page_directory,F,i,PERM_USER & ~PERM_PRESENT);
							}
...
int freeSharedObject(int32 sharedObjectID, void *startVA)
{
	...
	//	1) Get the shared object from the "shares" array (use get_share_object_ID())
		struct Share* sharedObject =&shares[sharedObjectID];
		if(sharedObject == NULL) return E_SHARED_MEM_NOT_EXISTS;
	//	2) Unmap it from the current environment "myenv"
		int size = ROUNDUP(sharedObject->size,PAGE_SIZE);
		uint32 VA= (uint32)startVA;
	    uint32 end= ROUNDUP( VA+size,PAGE_SIZE );
		for(uint32 va = VA; va <end;va+=PAGE_SIZE)
		{
	...
	//	5) If this is the last share, delete the share object (use free_share_object())
		if(sharedObject->references==0)
		{
			int res = free_share_object(sharedObjectID);
			//cprintf("%d\n",res);
			if(res != 0)
				return E_SHARED_MEM_NOT_EXISTS;
		}
