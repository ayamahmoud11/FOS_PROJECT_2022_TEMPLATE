...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	int index= get_semaphore_object_ID(ownerEnvID,semaphoreName);
	   for(int i= 0; i< MAX_SEMAPHORES; i++){
		   if (semaphoreName==semaphores[i].name)
		   {
			   return i;
		   }
		     ...
		     if(index!=E_SEMAPHORE_EXISTS)

		{

	             struct Semaphore *allocatedObject= NULL;
	              index=allocate_semaphore_object(&allocatedObject);
				 semaphores[index].ownerID=ownerEnvID;
				 strcpy(semaphores[index].name,semaphoreName);
				 semaphores[index].value=initialValue;
		...
		if(index!=E_SEMAPHORE_EXISTS)
		{
		     ...
		     struct Env* myenv = curenv;
		        ...
		        semaphores[index].value--;
		    	  ...
		    	  struct Env_Queue *ptr_queue= &semaphores[index].env_queue;
		    	   enqueue(ptr_queue, myenv);
